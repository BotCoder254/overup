...
| Webhook deliveries show `401` | `GITHUB_WEBHOOK_SECRET` doesn't match the App's secret. **Read the `cause=` field on the `webhook delivery failed signature verification` warn** — it names the fault: `mismatch` = the values genuinely differ (retype both sides; watch for a trailing newline or wrapping `"` quotes pasted into the env panel — the backend trims whitespace at load but cannot strip quotes for you, and warns at boot if it sees them); `missing_header` = the App has **no** webhook secret set, so GitHub sends no signature at all; `bad_prefix`/`invalid_hex` = something other than GitHub is posting to the endpoint. |
| Webhook deliveries show `401` right after an upgrade, or the container won't boot with `GITHUB_WEBHOOK_SECRET must be at least 16 bytes after trimming` | Intentional: the length floor is now measured **after** trimming, so a short secret padded with spaces that used to boot is now correctly rejected. Generate a real one (`openssl rand -hex 32`) and set it on both the env panel and the GitHub App. Same rule applies to `RUNNER_JOB_SIGNING_KEY` (≥32 bytes). |
| Webhook deliveries show `404` | Wrong Webhook URL — it's `/webhooks/github`, not under `/api`. |
| Webhook deliveries show `413`, or large pushes silently never arrive | Payload exceeded the 1 MiB body limit on `/webhooks/github` (GitHub allows up to 25 MB). Rejected before the handler, so there is **no** warn line — distinct from the 401 path. |
| Webhook deliveries show `429` during a burst | Per-IP rate limit (10/s, burst 20). Behind a proxy without `TRUST_PROXY=true`, every GitHub sender collapses into one bucket — set it if exactly one trusted reverse proxy fronts the API. |
