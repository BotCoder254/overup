...
| [fix-empty-workspace-stale-runner.md](./fix-empty-workspace-stale-runner.md) | **Runbook** for jobs failing with an empty `/workspace` (`ENOENT package.json`) because the deployed runner image is older than the repo: the log-order fingerprint that identifies the stale binary, rebuild-from-the-right-source + in-image `grep` verification, registry-side layer forensics without Docker, the revoke-and-recreate rollout, plus the two look-alike failures — containerd `Lchown` layer-extraction errors (host healing) and `cargo: not found` (per-toolchain job images via `container:`). |
